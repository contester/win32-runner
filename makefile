...
CXX := g++.exe
CPPFLAGS := -pipe -Wall -g -Os -mno-cygwin -I$(BOOST)/include/boost-$(BOOSTVER) -I$(PROTOBUF)/include -I$(RUNLIB32)/include -D_WIN32_WINNT=0x0501 -DBOOST_THREAD_USE_LIB -Isrc/
LDFLAGS := -pipe -Wl,--allow-multiple-definition -Wall -g -Os -mno-cygwin -lws2_32 -L$(BOOST)/lib -lboost_date_time-mgw45-mt-$(BOOSTVER) -lboost_system-mgw45-mt-$(BOOSTVER) -lboost_thread-mgw45-mt-$(BOOSTVER) -L$(PROTOBUF)/lib -lprotobuf -L$(RUNLIB32)/build -lrunlib32-static -ladvapi32 -lpsapi -luserenv -lmswsock -lrunlib32-static
